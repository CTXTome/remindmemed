...
class _MainScreenState extends State<MainScreen> {
  ...
  void initState() {
    super.initState();
    BlocProvider.of<MedicationBloc>(context).add(LoadMedication());
                ...
                builder: (context, state) {
                  if (state is MedicationLoading) {
                    return CircularProgressIndicator();
                      ...
                      final nextMedication = state.medications[0]; // or however you determine the "next" medication
                      return Card(
                        child: ListTile(
                          title: Text("Next Med: ${nextMedication.name}"),
                          subtitle: Text("@ ${DateFormat('h:mm a').format(nextMedication.time)}"),
                          ),
                          );
                          } else {
